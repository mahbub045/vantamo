...
async function seed() {
  ...
  for (const u of users) {
    db.prepare('INSERT INTO users (id, name, email, password_hash, role, avatar) VALUES (?,?,?,?,?,?)')
      .run(u.id, u.name, u.email, passwordHash, u.role, u.avatar);
  }
  for (const c of clients) {
    db.prepare('INSERT INTO clients (id, name, company, contact_person, email, phone, notes) VALUES (?,?,?,?,?,?,?)')
      .run(c.id, c.name, c.company, c.contact_person, c.email, c.phone, c.notes);
  }
  for (const p of projects) {
    db.prepare('INSERT INTO projects (id, name, client_id, status, priority, description, start_date, due_date, progress) VALUES (?,?,?,?,?,?,?,?,?)')
      .run(p.id, p.name, p.client_id, p.status, p.priority, p.description, p.start_date, p.due_date, p.progress);
  }
  for (const t of tasks) {
    db.prepare('INSERT INTO tasks (id, title, description, project_id, assignee_id, status, priority, due_date, position) VALUES (?,?,?,?,?,?,?,?,?)')
      .run(t.id, t.title, t.description, t.project_id, t.assignee_id, t.status, t.priority, t.due_date, t.position);
  }
  ...
  for (const a of activities) {
    db.prepare('INSERT INTO activities (id, type, user_id, entity_id, entity_type, description, metadata) VALUES (?,?,?,?,?,?,?)')
      .run(uuid(), a.type, users[0].id, a.entity_id, a.entity_type, a.description, null);
  }
