...
export class SqliteClientRepo {
  ...
  findAll(archived = false): Client[] {
    return this.db.prepare('SELECT * FROM clients WHERE archived = ? ORDER BY name').all(archived ? 1 : 0) as Client[];
  }
  ...
  create(client: Omit<Client, 'created_at' | 'updated_at' | 'archived'>): Client {
    ...
    ).run(id, client.name, client.company, client.contact_person, client.email, client.phone, client.notes);
