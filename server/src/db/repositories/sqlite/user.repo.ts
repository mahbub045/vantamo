...
import type { IUserRepository } from '../../interfaces'; // adjusted path below
...
export class SqliteUserRepo implements IUserRepository {
  ...
  findById(id: string): UserType | null {
    return this.db
      ...
      .get(id) as UserType | null;
  }

  findByEmail(email: string): UserType | null {
    return this.db
      .prepare('SELECT * FROM users WHERE email = ?')
      .get(email) as UserType | null;
  }

  findAll(): UserType[] {
    return this.db
      .prepare(
        'SELECT id, name, email, role, avatar, created_at, updated_at FROM users ORDER BY name',
      )
      .all() as UserType[];
  }
