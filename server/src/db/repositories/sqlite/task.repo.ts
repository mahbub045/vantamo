...
export class SqliteTaskRepo {
  ...
  search(query: string): Task[] {
    const q = `%${query}%`;
    return this.db.prepare('SELECT * FROM tasks WHERE title LIKE ? OR description LIKE ? ORDER BY created_at DESC').all(q, q) as Task[];
  }

  create(task: Omit<Task, 'created_at' | 'updated_at'>): Task {
    const id = task.id || uuid();
    const maxPos = this.db.prepare('SELECT COALESCE(MAX(position),0)+1 as p FROM tasks WHERE status = ? AND project_id = ?')
    ...
    ).run(id, task.title, task.description, task.project_id, task.assignee_id, task.status, task.priority, task.due_date, task.position ?? maxPos.p);
  ...
  updatePosition(id: string, status: string, position: number): Task | null {
    this.db.prepare("UPDATE tasks SET status = ?, position = ?, updated_at = datetime('now') WHERE id = ?")
