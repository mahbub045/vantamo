...
export class SqliteProjectRepo {
  ...
  search(query: string): Project[] {
    const q = `%${query}%`;
    return this.db.prepare('SELECT * FROM projects WHERE name LIKE ? OR description LIKE ? ORDER BY name').all(q, q) as Project[];
  }
  ...
  create(project: Omit<Project, 'created_at' | 'updated_at' | 'progress'>): Project {
    ...
    ).run(id, project.name, project.client_id, project.status, project.priority, project.description, project.start_date, project.due_date);
  ...
  countByStatus(): Record<string, number> {
    const rows = this.db.prepare('SELECT status, COUNT(*) as count FROM projects GROUP BY status').all() as { status: string; count: number }[];
